...
class TestStuff(TestCase):
    ...
    def tests_friendly_boolean_anything_else(self):
        expected_value = 'No'
        actual_value = ct.friendly_boolean('junk')

        self.assertEqual(actual_value, expected_value)

    def tests_pluralise_unit_singular_with_s(self):
        expected_value = 'gram'
        actual_value = ct.pluralise_unit('gram(s)', '1')

        self.assertEqual(actual_value, expected_value)

    def tests_pluralise_unit_singular_wihtout_s(self):
        expected_value = 'gram'
        actual_value = ct.pluralise_unit('gram', '1')

        self.assertEqual(actual_value, expected_value)

    def tests_pluralise_not_singular(self):
        expected_value = 'number of objects'
        actual_value = ct.pluralise_unit('number of objects', '2')

        self.assertEqual(actual_value, expected_value)

    def tests_str_date(self):
