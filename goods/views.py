...
class GoodsDetailEmpty(TemplateView):
    def get(self, request, **kwargs):
        return redirect(
            reverse_lazy(
                "goods:good_detail", kwargs={"pk": kwargs["pk"], "type": "case-notes"}
            )
        )
...
class GoodsDetail(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        case_note_notifications = len(
            [
                ...
                if str(x["parent"]) == self.good_id and x["object_type"] == "case_note"
            ]
        )
        ecju_query_notifications = len(
            [
                x
                for x in notifications
                if str(x["parent"]) == self.good_id and x["object_type"] == "ecju_query"
            ]
        )
        ...
        if self.view_type == "ecju-queries":
            ...
            ) = get_application_ecju_queries(request, self.good["case_id"])
    ...
    def post(self, request, **kwargs):
        ...
        response, _ = post_application_case_notes(
            request, data["case_id"], request.POST
        )
        ...
        return redirect(
            reverse_lazy(
                "goods:good_detail", kwargs={"pk": good_id, "type": "case-notes"}
            )
        )
    ...
    def post(self, request):
        ...
        if "errors" in validated_data:
            if validated_data["errors"]:
                return form_page(
                    request, self.form, data=data, errors=validated_data.get("errors")
                )

        return redirect(
            reverse_lazy(
                "goods:attach_documents", kwargs={"pk": validated_data["good"]["id"]}
            )
        )
...
class RaiseCLCQuery(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if "errors" in data:
            return form_page(
                request,
                forms.are_you_sure(str(kwargs["pk"])),
                data=request_data,
                errors=data["errors"],
            )
...
class RespondToQuery(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.ecju_query = get_ecju_query(
            request, self.clc_query_case_id, self.ecju_query_id
        )
        ...
        if self.ecju_query["response"]:
            return redirect(
                reverse_lazy(
                    "goods:good_detail",
                    kwargs={"pk": self.good_id, "type": "ecju-queries"},
                )
            )
    ...
    def post(self, request, **kwargs):
        ...
        if form_name == "respond_to_query":
            ...
            response, status_code = put_ecju_query(
                request, self.clc_query_case_id, self.ecju_query_id, data
            )
            ...
            if status_code != 200:
                ...
                form = ecju_query_respond_confirmation_form(
                    reverse_lazy(
                        "goods:respond_to_query",
                        kwargs={"pk": self.good_id, "query_pk": self.ecju_query_id},
                    )
                )
            ...
            if request.POST.get("confirm_response") == "yes":
                data, status_code = put_ecju_query(
                    request, self.clc_query_case_id, self.ecju_query_id, request.POST
                )
                if "errors" in data:
                    return form_page(
                        request,
                ...
                return form_page(
                    request,
                    respond_to_query_form(self.clc_query_case_id, self.ecju_query),
                    data=request.POST,
                )
            else:
