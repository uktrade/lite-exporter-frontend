...
def add_goods_questions(allow_query=True, back_link=BackLink(), prefix=""):
    if allow_query:
        ...
        is_your_good_controlled_options = [
            ...
            Option(key="no", value=get_const_string("GOODS_CREATE_CONTROL_CODE_NO")),
            Option(
                key="unsure", value=get_const_string("GOODS_CREATE_CONTROL_CODE_UNSURE")
            ),
        ]
    else:
        description = get_const_string(
    ...
    form = Form(
        title=get_const_string("GOODS_CREATE_TITLE"),
        questions=[
            TextArea(
...
def are_you_sure(good_id):
    return Form(
        ...
        back_link=BackLink(
            "Back to good", reverse("goods:good", kwargs={"pk": good_id})
        ),
    )
...
def attach_documents_form(back_url, description):
    return Form(
        ...
        [
            ...
            TextArea(
                title=get_string(
                    "goods.documents.attach_documents.description_field_title"
                ),
                optional=True,
                name="description",
                extras={"max_length": 280,},
        ...
        back_link=BackLink(
            get_string("goods.documents.attach_documents.back_to_good"), back_url
        ),
    )
...
def respond_to_query_form(good_id, ecju_query):
    return Form(
        ...
        back_link=BackLink(
            "Back to good",
            reverse_lazy(
                "goods:good_detail", kwargs={"pk": good_id, "type": "ecju-queries"}
            ),
        ),
...
def delete_good_form(good):
    return Form(
        ...
        buttons=[
            Button(
                value="Yes, delete the good", action="submit", style=ButtonStyle.WARNING
            ),
            Button(
                value="Cancel",
                action="",
