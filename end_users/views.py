...
from end_users.services import (
    get_end_user_advisories,
    post_end_user_advisories,
    get_end_user_advisory,
)
...
class CopyAdvisory(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.forms.forms[0].questions.append(
            HiddenField("end_user.sub_type", query["end_user"]["sub_type"]["key"])
        )
    ...
    def post(self, request, **kwargs):
        response, data = submit_paged_form(
            request, self.forms, post_end_user_advisories, inject_data=self.data
        )

        if response:
            return response

        return end_user_advisory_success_page(
            request, str(data["end_user_advisory"]["id"])
        )
...
class ApplyForAnAdvisory(TemplateView):
    ...
    def post(self, request, **kwargs):
        response, data = submit_paged_form(
            request, self.forms, post_end_user_advisories
        )
...
class EndUserDetailEmpty(TemplateView):
    def get(self, request, **kwargs):
        return redirect(
            reverse_lazy(
                "end_users:end_user_detail",
                kwargs={"pk": kwargs["pk"], "type": "case-notes"},
            )
        )
...
class EndUserDetail(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.end_user_advisory, self.case_id = get_end_user_advisory(
            request, self.end_user_advisory_id
        )
    ...
    def get(self, request, **kwargs):
        ...
        case_note_notifications = len(
            [
                ...
                if str(x["parent"]) == self.end_user_advisory_id
        ...
        if self.view_type == "ecju-queries":
            (
                context["open_queries"],
                context["closed_queries"],
            ) = get_application_ecju_queries(request, self.case_id)
    ...
    def post(self, request, **kwargs):
        ...
        return redirect(
            reverse_lazy(
                "end_users:end_user_detail",
                kwargs={"pk": self.end_user_advisory_id, "type": "case-notes"},
            )
        )
...
class RespondToQuery(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.ecju_query["response"]:
            return redirect(
                reverse_lazy(
                    "end_users:end_user_detail",
                    kwargs={"pk": self.end_user_advisory_id, "type": "ecju-queries"},
                )
            )
    ...
    def get(self, request, **kwargs):
        ...
        """
        return form_page(
            request, respond_to_query_form(self.end_user_advisory_id, self.ecju_query)
        )
    ...
    def post(self, request, **kwargs):
        ...
        if form_name == "respond_to_query":
            ...
            response, status_code = put_ecju_query(
                request, self.case_id, self.ecju_query_id, data
            )
            ...
            if request.POST.get("confirm_response") == "yes":
                data, status_code = put_ecju_query(
                    request, self.case_id, self.ecju_query_id, request.POST
                )
                if "errors" in data:
                    return form_page(
                        request,
                        respond_to_query_form(
                            self.end_user_advisory_id, self.ecju_query
                        ),
                        data=request.POST,
                        errors=data["errors"],
                    )
            ...
            return error_page(
                None, "We had an issue creating your response. Try again later."
            )
