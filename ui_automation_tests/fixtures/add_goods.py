...
def add_good_to_application(driver, context, lite_client):
    ...
    no = utils.get_element_index_by_text(
        elements, context.goods_name, complete_match=False
    )
    driver.find_elements_by_css_selector(".govuk-table__row .govuk-link")[
        no - 1
    ].click()
    application_goods_list = ApplicationGoodsList(driver)
    context.unit = "Number of articles"
    context.value = "11"
    application_goods_list.add_values_to_good(
        str(context.value), str(context.value), context.unit
    )
...
def add_an_incorporated_good_to_application(
    driver, request, context, exporter_url, seed_data_config
):
...
@fixture(scope="function")
def add_a_non_incorporated_good_to_application(
    driver, context, request, exporter_url, seed_data_config
):
    lite_client = get_lite_client(context, seed_data_config=seed_data_config)
