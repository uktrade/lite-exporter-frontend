...
@fixture(scope="session")
def register_organisation_for_switching_organisation(
    driver, request, context, seed_data_config
):
    lite_client = get_lite_client(context, seed_data_config=seed_data_config)
    lite_client.seed_org.setup_org_for_switching_organisations()
    context.org_name_for_switching_organisations = lite_client.context[
