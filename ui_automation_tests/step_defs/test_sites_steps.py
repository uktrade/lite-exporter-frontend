...
@when(
    parsers.parse(
        'I enter in text for new site "{edited}" "{address}" "{postcode}" "{city}" '
        '"{region}" and "{country}"'
    )
)
def new_sites_info(driver, edited, address, postcode, city, region, country, context):
    ...
    new_site.enter_info_for_new_site(
        new_site_name, address, postcode, city, region, country
    )
...
def assert_checkbox_at_position(driver, no, checked):
    ...
    if checked == "checked":
        ...
        assert (
            sites_page.get_checked_attribute_of_sites_checkbox(int(no) - 1)
            is not "true"
        )
