...
def i_remove_an_additional_document(driver):
    remove_consignee_link = ApplicationOverviewPage(
        driver
    ).find_remove_additional_document_link()
    driver.execute_script("arguments[0].click();", remove_consignee_link)
...
def no_documents_are_set_on_the_application(driver):
    assert (
        ApplicationOverviewPage(driver).find_remove_additional_document_link(),
        None,
    )
...
def assert_ref_name(context, driver):
    assert (
        context.app_name in driver.find_element_by_css_selector(".lite-task-list").text
    )
