...
def i_remove_an_ultimate_end_user(driver):
    ...
    assert (
        total == 1
    ), "total on the ultimate end users summary is incorrect after removing ultimate end user"
    app = ApplicationOverviewPage(driver)
    app.click_on_back_to_overview_text()
...
def click_add_to_application_button(driver, no, context):
    num = int(no) - 1
    context.goods_name = ApplicationGoodsList(
        driver
    ).get_text_of_gov_heading_within_card(num)
