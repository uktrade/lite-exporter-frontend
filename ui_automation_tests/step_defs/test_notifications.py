...
def notification_on_application_list(driver, context):
    ...
    no = helpers.get_element_index_by_text(
        elements, context.app_name, complete_match=False
    )
...
def internal_note_visible(driver, context):
    assert (
        context.case_note_text
        in driver.find_element_by_css_selector(".lite-case-notes").text
    )
...
def notification_does_not_exist(driver, context):
    if context.number_of_notifications != 1:
        number_of_notifications_after_acknowledgment = Hub(
            driver
        ).return_number_of_notifications()
        # Commenting out due to bug LT-1433
