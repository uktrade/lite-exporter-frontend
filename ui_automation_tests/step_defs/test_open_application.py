...
def i_see_open_licence_error(driver):
    ...
    assert (
        "Cannot create an application with no good descriptions attached"
        in shared.get_text_of_error_messages()
    )
    assert (
        "Cannot create an application with no sites or external sites attached"
        in shared.get_text_of_error_messages()
    )
    assert (
        "Cannot create an application without countries being set"
        in shared.get_text_of_error_messages()
    )
...
@then(
    parsers.parse(
        'I see my goods type added at position "{position}" with a description and a control code'
    )
)
...
@then(
    "I see my goods type added to the overview page with a description and a control code"
)
...
def search_country_result(driver, country):
    assert (
        country == ApplicationCountriesList(driver).get_text_of_countries_list()
