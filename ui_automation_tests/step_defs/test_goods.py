...
def i_see_the_attached_good(driver, context):
    ...
    assert (
        context.document_description in added_doc
    ), "file description is not displayed"
...
@when(
    parsers.parse('I attach a document to the good with description "{description}"')
)  # noqa
def i_attach_a_document_to_the_good(driver, description):
    ...
    file_to_upload_abs_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), os.pardir, "resources", file_to_be_deleted_name
        )
    )
...
def create_a_new_good_in_application(
    driver, description, controlled, control_code, incorporated, part_number
):
    prefix = "good_"
...
def i_enter_detail_for_the_good_on_the_application(driver, value, quantity, unit):
    ApplicationGoodsList(driver, prefix="good_on_app_").add_values_to_good(
        value, quantity, unit
    )
    Shared(driver).click_continue()
