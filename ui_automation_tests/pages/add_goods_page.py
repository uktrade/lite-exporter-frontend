class AddGoodPage:
    ...
    def select_is_your_good_controlled(self, option):
        ...
        self.driver.find_element_by_id(
            self.prefix + "is_good_controlled-%s" % option.lower()
        ).click()
    ...
    def enter_control_code_unsure(self, code):
        control_code_tb = self.driver.find_element_by_id(
            self.prefix + "not_sure_details_control_code"
        )
        control_code_tb.clear()
        control_code_tb.send_keys(code)

    def enter_control_unsure_details(self, details):
        unsure_details = self.driver.find_element_by_id(
            self.prefix + "not_sure_details_details"
        )
    ...
    def select_control_unsure_confirmation(self, option):
        ...
        self.driver.find_element_by_id(
            self.prefix + "clc_query_confirmation-" + option.lower()
        ).click()

    def select_is_your_good_intended_to_be_incorporated_into_an_end_product(
        self, option
    ):
        if option == "Yes":
            self.driver.find_element_by_id(
                self.prefix + "is_good_end_product-no"
            ).click()
        else:
            self.driver.find_element_by_id(
                self.prefix + "is_good_end_product-yes"
            ).click()
