...
class ApplicationGoodsList:
    def __init__(self, driver, prefix=""):
        self.driver = driver
        self.prefix = prefix
        self.add_from_org_goods_button = (
            'a.govuk-button[href*="add_preexisting"]'  # CSS
        )
    ...
    def add_values_to_good(self, value, quantity, unit):
        ...
        self.driver.find_element_by_id(self.prefix + self.quantity_field).send_keys(
            quantity
        )
        select = Select(
            self.driver.find_element_by_id(self.prefix + self.unit_dropdown)
        )
    ...
    def type_into_filter_description_search_box_and_filter(self, value):
        ...
        self.driver.find_element_by_id(self.filter_description_search_box).send_keys(
            value
        )
    ...
    def type_into_filter_part_number_search_box_and_filter(self, value):
        self.driver.find_element_by_id(self.filter_part_number_search_box).send_keys(
            value
        )
        self.driver.find_element_by_xpath(self.filter_button).click()

    def type_into_filter_control_rating_search_box_and_filter(self, value):
        self.driver.find_element_by_id(self.filter_control_rating_search_box).send_keys(
            value
        )
        self.driver.find_element_by_xpath(self.filter_button).click()
