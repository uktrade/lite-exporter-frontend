...
class ViewUser(TemplateView):
    def get(self, request, **kwargs):
        user = get_organisation_user(
            request, str(request.user.organisation), str(kwargs["pk"])
        )["user"]
...
class ViewProfile(TemplateView):
    def get(self, request, **kwargs):
        user = request.user
        return redirect(
            reverse_lazy("users:user", kwargs={"pk": user.lite_api_user_id})
        )
...
class ChangeUserStatus(TemplateView):
    def get(self, request, **kwargs):
        ...
        if status == "deactivate":
            description = (
                "This user will no longer be able to log in or perform tasks on LITE "
                "on behalf of your organisation."
            )

        if status == "reactivate":
            description = (
                "This user will be able to log in to and perform tasks on LITE on behalf "
    ...
    def post(self, request, **kwargs):
        ...
        put_organisation_user(
            request, str(request.user.organisation), str(kwargs["pk"]), request.POST
        )
