...
def get_draft_applications(request):
    ...
    return (
        data.json().get("applications") if data.status_code == HTTPStatus.OK else None
    )
...
def get_ultimate_end_user_document(request, pk, ueu_pk):
    data = get(
        request, APPLICATIONS_URL + pk + ULTIMATE_END_USER_URL + ueu_pk + DOCUMENT_URL
    )
    return data.json(), data.status_code
...
def post_ultimate_end_user_document(request, pk, ueu_pk, json):
    data = post(
        request,
        APPLICATIONS_URL + pk + ULTIMATE_END_USER_URL + ueu_pk + DOCUMENT_URL,
        json,
    )
    return data.json(), data.status_code
...
def get_third_party_document(request, pk, tp_pk):
    data = get(
        request, APPLICATIONS_URL + pk + THIRD_PARTIES_URL + tp_pk + DOCUMENT_URL
    )
    return data.json(), data.status_code
...
def post_third_party_document(request, pk, tp_pk, json):
    data = post(
        request, APPLICATIONS_URL + pk + THIRD_PARTIES_URL + tp_pk + DOCUMENT_URL, json
    )
    return data.json(), data.status_code
...
def delete_additional_party_document(request, pk, doc_pk):
    data = delete(
        request, APPLICATIONS_URL + pk + ADDITIONAL_DOCUMENT_URL + doc_pk + "/"
    )
    return data.status_code
...
def delete_application_preexisting_good(request, good_on_application_pk):
    response = delete(
        request, APPLICATIONS_URL + "good-on-application/" + good_on_application_pk
    )
...
def get_ecju_query(request, pk, query_pk):
    data = get(request, CASES_URL + pk + ECJU_QUERIES_URL + query_pk).json()[
        "ecju_query"
    ]
    return data
