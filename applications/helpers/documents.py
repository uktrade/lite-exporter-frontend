...
def _validate_document_upload_files(files):
    if not files:
        return strings.applications.AttachDocumentPage.NO_FILE_ERROR
    elif len(files) != 1:
        return strings.applications.AttachDocumentPage.MULTIPLE_FILES
    else:
        return None
...
def handle_document_upload(request):
    ...
    try:
        ...
        if request.upload_handlers[0].content_length > MAX_UPLOAD_SIZE:
            return None, strings.applications.AttachDocumentPage.FILE_TOO_LARGE
        else:
            return None, strings.applications.AttachDocumentPage.UPLOAD_FAILURE_ERROR
