...
class EndUser(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        response, _ = submit_paged_form(
            request, new_end_user_forms(), post_end_user, object_pk=draft_id
        )
        ...
        if draft.get("licence_type").get("key") == STANDARD_LICENCE:
            return redirect(
                reverse_lazy(
                    "applications:end_user_attach_document", kwargs={"pk": draft_id}
                )
            )
        else:
...
class AddUltimateEndUser(TemplateView):
    ...
    def post(self, request, **kwargs):
        response, data = submit_paged_form(
            request, self.form, post_ultimate_end_user, object_pk=self.draft_id
        )
...
class RemoveUltimateEndUser(TemplateView):
    def get(self, request, **kwargs):
        ...
        return redirect(
            reverse_lazy("applications:ultimate_end_users", kwargs={"pk": draft_id})
        )
