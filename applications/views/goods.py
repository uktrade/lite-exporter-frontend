...
class AddNewGood(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        return form_page(
            request, self.form, self.data, extra_data={"form_pk": self.form_num}
        )
    ...
    def post(self, request, **kwargs):
        ...
        if request.POST.get("_action", None) == "back":
            ...
            if request.FILES:
                ...
                self.errors = {
                    "documents": [
                        get_const_string("APPLICATION_GOODS_ADD_DOCUMENT_MISSING")
                    ]
                }

        return form_page(
            request, self.form, self.data, self.errors, {"form_pk": self.form_num}
        )
    ...
    def handle_post_for_form(self, request, form_num, pk=None):
        ...
        if data.status_code != HTTPStatus.OK:
            self.data = post
            self.generate_form(request, form_num)
            self.errors = self.add_prefix_to_errors(
                data.json()["errors"], self.prefix[form_num]
            )
        else:
            self.data = post
            generate_form_num = form_num + 1
...
class RemovePreexistingGood(TemplateView):
    def get(self, request, **kwargs):
        ...
        status_code = delete_application_preexisting_good(
            request, good_on_application_id
        )
        ...
        return redirect(
            reverse_lazy("applications:goods", kwargs={"pk": application_id})
        )
