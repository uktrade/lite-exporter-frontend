...
class AddThirdParty(TemplateView):
    ...
    def post(self, request, **kwargs):
        response, data = submit_paged_form(
            request, self.form, post_third_party, object_pk=self.draft_id
        )
...
class RemoveThirdParty(TemplateView):
    def get(self, request, **kwargs):
        ...
        return redirect(
            reverse_lazy("applications:third_parties", kwargs={"pk": draft_id})
        )
...
class Consignee(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        response, _ = submit_paged_form(
            request, new_consignee_forms(), post_consignee, object_pk=draft_id
        )
        ...
        return redirect(
            reverse_lazy(
                "applications:consignee_attach_document", kwargs={"pk": draft_id}
            )
        )
