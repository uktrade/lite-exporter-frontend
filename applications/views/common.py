    ...
    def get(self, request, **kwargs):
        ...
        return redirect(
            reverse_lazy(
                ...
                kwargs={"pk": application_id, "type": "case-notes"},
            )
        )
...
class DeleteApplication(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        url_with_query_params = (
            f"?application_deleted={(str(status == HTTPStatus.OK)).lower()}"
        )
        return redirect(
...
class ApplicationEditType(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if edit_type == "major":
            data, status_code = set_application_status(
                request, str(kwargs["pk"]), "applicant_editing"
            )

            if status_code != HTTPStatus.OK:
                return form_page(
                    request, edit_type_form(str(kwargs["pk"])), errors=data
                )
        ...
        return redirect(
            reverse_lazy("applications:edit", kwargs={"pk": str(kwargs["pk"])})
        )
...
class ApplicationEditOverview(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if status_code != HTTPStatus.OK:
            return get_licence_overview(
                request, application=application_data, errors=submit_data.get("errors")
            )
...
class ApplicationDetail(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        case_note_notifications = len(
            [
                ...
                if x["parent"] == self.application_id
    ...
    def post(self, request, **kwargs):
        ...
        return redirect(
            reverse_lazy("applications:application", kwargs={"pk": self.application_id})
        )
...
class RespondToQuery(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.ecju_query = get_ecju_query(
            request, str(kwargs["pk"]), str(kwargs["query_pk"])
        )
        ...
        if self.ecju_query["response"]:
            return redirect(
                reverse_lazy(
                    "applications:application-detail",
                    kwargs={"pk": self.application_id, "type": "ecju-queries"},
                )
            )
    ...
    def get(self, request, **kwargs):
        ...
        """
        return form_page(
            request, respond_to_query_form(self.application_id, self.ecju_query)
        )
    ...
    def post(self, request, **kwargs):
        ...
        if form_name == "respond_to_query":
            ...
            response, status_code = put_ecju_query(
                request, self.application_id, self.ecju_query_id, data
            )
            ...
            if request.POST.get("confirm_response") == "yes":
                data, status_code = put_ecju_query(
                    request, self.application_id, self.ecju_query_id, request.POST
                )
