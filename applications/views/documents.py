...
from applications.forms.end_user import (
...
def get_upload_page(path, draft_id):
    ...
    if paths["description"]:
        description_text = get_string(
            paths["strings"] + ".attach_documents.description_field_title"
        )
    else:
        description_text = None

    return attach_document_form(
        draft_url=reverse(paths["homepage"], kwargs={"pk": draft_id}),
...
def get_homepage(request, draft_id):
    return redirect(
        reverse(get_page_content(request.path)["homepage"], kwargs={"pk": draft_id})
    )
...
class AttachDocuments(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if error:
            return error_page(
                None, get_string("end_user.documents.attach_documents.upload_error")
            )

        if "ultimate-end-user" in request.path:
            _, status_code = post_ultimate_end_user_document(
                request, draft_id, str(kwargs["ueu_pk"]), data
            )
            ...
            _, status_code = post_third_party_document(
                request, draft_id, str(kwargs["tp_pk"]), data
            )
...
class DownloadDocument(TemplateView):
    def get(self, request, **kwargs):
        draft_id = str(kwargs["pk"])
        if "ultimate-end-user" in request.path:
            document, _ = get_ultimate_end_user_document(
                request, draft_id, str(kwargs["ueu_pk"])
            )
        elif "consignee" in request.path:
            document, _ = get_consignee_document(request, draft_id)
        elif "third-parties" in request.path:
            document, _ = get_third_party_document(
                request, draft_id, str(kwargs["tp_pk"])
            )
        elif "end-user" in request.path:
            document, _ = get_end_user_document(request, draft_id)
        elif "additional-document" in request.path:
            document, _ = get_additional_document(
                request, draft_id, str(kwargs["doc_pk"])
            )
        else:
            return error_page(
                None, get_string("end_user.documents.attach_documents.download_error")
            )
...
class DeleteDocument(TemplateView):
    def get(self, request, **kwargs):
        return form_page(
            request, get_delete_confirmation_page(request.path, str(kwargs["pk"]))
        )
    ...
    def post(self, request, **kwargs):
        ...
        if option is None:
            ...
            if option == "yes":
                if "ultimate-end-user" in request.path:
                    status_code = delete_ultimate_end_user_document(
                        request, draft_id, str(kwargs["ueu_pk"])
                    )
                elif "consignee" in request.path:
                    status_code = delete_consignee_document(request, draft_id)
                elif "third-parties" in request.path:
                    status_code = delete_third_party_document(
                        request, draft_id, str(kwargs["tp_pk"])
                    )
                elif "end-user" in request.path:
                    status_code = delete_end_user_document(request, draft_id)
                elif "additional-document" in request.path:
                    status_code = delete_additional_party_document(
                        request, draft_id, str(kwargs["doc_pk"])
                    )
                else:
                    return error_page(
                        None,
                        get_string("end_user.documents.attach_documents.delete_error"),
                    )
