...
class Location(TemplateView):
    def get(self, request, **kwargs):
        ...
        if external_locations["external_locations"]:
            ...
            if application["status"].get("key") == "submitted":
                return redirect(
                    reverse_lazy(
                        "applications:external_locations", kwargs={"pk": application_id}
                    )
                )
        else:
            ...
            if application["status"].get("key") == "submitted":
                return redirect(
                    reverse_lazy(
                        "applications:existing_sites", kwargs={"pk": application_id}
                    )
                )
    ...
    def post(self, request, **kwargs):
        ...
        if data["organisation_or_external"] == "external":
            return redirect(
                reverse_lazy("applications:external_locations", kwargs={"pk": draft_id})
            )
        else:
            return redirect(
                reverse_lazy("applications:existing_sites", kwargs={"pk": draft_id})
            )
...
class ExistingSites(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if status_code != 201:
            return form_page(
                request, sites_form(request), errors=response.get("errors"), data=data
            )
...
class ExternalLocations(TemplateView):
    def get(self, request, **kwargs):
        application_id = str(kwargs["pk"])
        application = get_application(request, application_id)
        org_external_locations, _ = get_external_locations(
            request, request.user.organisation
        )
        data, _ = get_external_locations_on_draft(request, application_id)

        if (
            application["status"].get("key") == "submitted"
            and not data["external_locations"]
        ):
            raise Http404
...
class AddExternalLocation(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if status_code == 400:
            return form_page(
                request, new_location_form(), data=request.POST, errors=data["errors"]
            )
...
class AddExistingExternalLocation(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        response, status_code = post_external_locations_on_draft(
            request, draft_id, data
        )

        if status_code != 201:
            return form_page(
                request, external_locations_form(request), errors=response.get("errors")
            )
