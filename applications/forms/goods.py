...
def good_on_application_form(good, units, title, prefix=""):
    return Form(
        ...
        questions=[
            ...
            CurrencyInput(
                title="What's the value of your goods?", name=prefix + "value"
            ),
            SideBySideSection(
                questions=[
                    QuantityInput(title="Quantity", name=prefix + "quantity"),
                    Select(
                        title="Unit of Measurement", name=prefix + "unit", options=units
                    ),
                ]
            ),
        ],
...
def add_new_good_forms(request, application_id):
    back_link = BackLink(
        get_const_string("APPLICATION_GOODS_ADD_BACK"),
    ...
    return [
        ...
        attach_documents_form(
            "#",
            description=get_const_string("APPLICATION_GOODS_ADD_DOCUMENT_DESCRIPTION"),
        ),
    ]
