...
class NewSite(TemplateView):
    ...
    def post(self, request, **kwargs):
        organisation_id = str(request.user.organisation)
        validated_data, _ = post_sites(
            request, organisation_id, nest_data(request.POST)
        )

        if "errors" in validated_data:
            validated_data["errors"] = flatten_data(validated_data["errors"])
            return form_page(
                request, self.form, data=request.POST, errors=validated_data["errors"]
            )
...
class EditSite(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.site["site"]["address"]["country"] = self.site["site"]["address"][
            "country"
        ]["id"]
